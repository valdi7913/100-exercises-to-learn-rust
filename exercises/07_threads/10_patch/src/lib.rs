    ...
    loop {
        match receiver.recv() {
            ...
            Ok(Command::Update {
                ...
                //Get ticket
                if let Option::Some(ticket) = store.get_mut(patch.id) {
                    if let Option::Some(title) = patch.title {
                        ticket.title = title;
                    }
                    if let Option::Some(description) = patch.description {
                        ticket.description = description;
                    }
                    if let Option::Some(status) = patch.status {
