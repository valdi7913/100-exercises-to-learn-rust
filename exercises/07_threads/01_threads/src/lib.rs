...
pub fn sum(v: Vec<i32>) -> i32 {
  let mid = v.len()/2;
  let v_left = v[..mid].to_owned();
  let v_right = v[mid..].to_owned();
  let handle1 = thread::spawn(move || v_left.iter().sum::<i32>());
  let handle2 = thread::spawn(move || v_right.iter().sum::<i32>());
